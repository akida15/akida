let handler = async (m, { conn, usedPrefix, command, args, isOwner, isAdmin, isROwner }) => {
  let isEnable = /true|تفعيل|(turn)?on|1/i.test(command)
...
`.trim()
  ...
  m.reply(`
*${type}* تم تفيعله *${isEnable ? '' : 'mati'}* ${isAll ? '' : isUser ? '' : 'untuk chat ini'}
`.trim())
}
handler.help = ['تفعيل'].map(v => v + '')
handler.tags = ['الاوامر']
handler.command = /^تفعيل|(tru|fals)e|(turn)?o(n|ff)|[01])$/i
